...
public class SemanticAnalysis implements EntityVisitor<Entity> {
    ...
    private void initVar(List<Variable> variables, Tuple init) {
        ...
        for (int i = 0; i < variables.size(); i++) {
            ...
            if (!assignable(v.type().must(), t, Optional.of(at.values().get(i)))) {
                semantic("init value not assignable : %s", v.pos());
                return;
            }
        }
    ...
    assignable(DerivedTypeDeclarer l, DerivedTypeDeclarer r,
        ...
        if (lr.kind() == ReferKind.PHANTOM) {
            var ok = re.match(e ->
                    new PhantomChecker(context).check(e));
            return ok && referable(lt, rt);
        }
        if (lr.kind() == ReferKind.STRONG) {
            return r.refer().has() &&
                    r.refer().get().checkType(ReferKind.STRONG)
                    && referable(lt, rt);
        }
    ...
    public Entity visit(AssertExpression e) {
        ...
        if (sr.get().kind() != er.get().kind()) return
                semantic("can't change refer kind from %s to %s: %s",
                        sr.get().kind(), er.get().kind(), e.type().pos());
        ...
        if (referable(eot.get(), sot.get()))
            return e;
        if (referable(sot.get(), eot.get()))
            return e;
    ...
    public Entity visit(CurrentExpression e) {
        if (enterMethod != null)
            return e;
        return semantic("use %s outside method", e.name());
    }
    ...
    private void checkNewMemSize(TypeDeclarer td) {
        if (td instanceof ArrayTypeDeclarer atd) {
            if (atd.length().has()) {
                checkNewMemSize(atd.element());
                return;
            }
            semantic("new mem type must specify length: %s",
                    td.pos());
            return;
        }

        if (td instanceof DerivedTypeDeclarer dtd) {
            for (var atd : dtd.definedType().generic().arguments()) {
                checkNewMemSize(atd);
            }
            return;
        }
    ...
    public Entity visit(NewExpression e) {
        ...
        if (e.type() instanceof NewMemType) {
            var len = e.arg().get();
            var right = typeDeducer.visit(len);
            if (typeDeducer.isInteger(right)) return e;
            return semantic("ram length must integer: %s", len.pos());
        }
        ...
        var right = typeDeducer.visit(e.arg().get());
        if (assignable(left, right, Optional.empty())) return e;
