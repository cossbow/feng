...
public class SemanticAnalyzer {
     ...
     */
    private Stream<GlobalVariable> findDepVars(
            IdentifierMap<GlobalVariable> all,
            Expression expr) {
        var result = Stream.<GlobalVariable>builder();
        ...
        while (!q.isEmpty()) {
            ...
            switch (c) {
                case SymbolExpression e -> {
                    // When initializing constants, this branch can only be taken because
                    // the AST only contains SymbolExpressions generated by parsing.
                    var s = e.symbol();
                    if (context.findFunc(s).has())
                        break;
                    if (s.module().has()) {
                        var o = context.findVar(s);
                        if (o.has()) break;
                        return semantic("can't find exported var '%s': %s", s, s.pos());
                    }
                    ...
                    if (ov.none()) {
                        ...
                        if (ot.match(t -> t instanceof EnumDefinition))
                            break;
                        return semantic("undeclared var '%s': %s", s, s.pos());
                    }
                    var v = ov.get();
                    result.add(v);
                }
    ...
    private Stream<GlobalVariable> findDepVars(
        ...
        if (td instanceof ArrayTypeDeclarer atd) {
            var set = findDepVars(all, atd.element());
            if (atd.length().none()) return set;
            return Stream.concat(set, findDepVars(all,
                    atd.length().get()));
        }
        if (td instanceof FuncTypeDeclarer ftd) {
            var p = ftd.prototype();
            return Stream.concat(p.parameterSet().types().stream(),
                            p.returnSet().stream())
                    .flatMap(t -> findDepVars(all, t));
        }
        return Stream.of();
    }

    private Set<GlobalVariable> findDepVars(
            IdentifierMap<GlobalVariable> all,
            GlobalVariable gv) {
        var ts = gv.type().stream().flatMap(t -> findDepVars(all, t));
        var vs = gv.value().stream().flatMap(v -> findDepVars(all, v));
        return Stream.concat(ts, vs).collect(Collectors.toSet());
    }
    ...
    globalGraph(Collection<GlobalVariable> list,
        ...
        for (var gv : list) {
            ...
            var deps = findDepVars(all, gv);
            for (var dep : deps) {
                if (isPrimitiveConst(dep)) continue;
    ...
    /**
     * Initialize {@link Primitive} consts
     */
    private List<GlobalVariable> globalConstInit(
            IdentifierMap<GlobalVariable> all) {
        var dag = globalGraph(
                all.values(), all);

        var list = new ArrayList<GlobalVariable>(all.size());
        for (var gv : dag) {
            if (!gv.isConst()) continue;
            if (gv.value().none())
                return semantic("const must initialized '%s': %s",
                        gv.name(), gv.pos());
            var deps = dag.prev(gv);
            // First, check whether the dependent variables are declared with `const`.
            for (var dv : deps) {
                if (dv.isConst()) break;
                return semantic("global const '%s' depends variable '%s': %s",
                        gv.name(), dv.name(), gv.pos());
            }
            // Then analyze its type
            if (gv.type().has()) {
                var td = analyse(gv.type().must());
                gv.type().set(td);
            } else {
                // Infer its type from the initial value
                var g = optimize(gv.value().must());
                gv.value().set(g.a());
                gv.type().set(g.b());
            }
            // Skip the non-primitive type constant
            if (!isPrimitiveConst(gv))
                continue;
            // Last, check whether the dependent variables are primitive type constant.
            for (var dv : deps) {
                if (isPrimitiveConst(dv)) continue;
                return semantic("global const '%s' depends variable '%s': %s",
                        gv.name(), dv.name(), gv.pos());
            }
     ...
     */
    private DAGGraph<GlobalVariable>
    globalVarInit(Set<GlobalVariable> list,
                  IdentifierMap<GlobalVariable> all) {
        var dag = globalGraph(list, all);
        if (dag.isEmpty()) return dag;
        for (var gv : dag) {
            if (gv.type().match(t -> t.checkRefer(PHANTOM))) {
                semantic("global variable can't be" +
                        " phantom reference: %s", gv.pos());
            }
        }
        dag.bfs(this::analyse);
        return dag;
    ...
    private boolean isPrimitiveConst(Variable gv) {
        return gv.isConst() && gv.type().match(t -> {
            if (t instanceof PrimitiveTypeDeclarer) return true;
            if (t instanceof LiteralTypeDeclarer lt) {
                return lt.literal().compatible().has();
            }
            return false;
        });
    }
    ...
    public AnalyseSymbolTable analyse() {
        ...
        result.constVars = globalConstInit(table.variables);
        ...
        // Class definition is the most complex, as it may rely
        ...
        var rest = subtract(table.variables.values(),
                result.constVars);
        result.dagVars = globalVarInit(rest, table.variables);
    ...
    private boolean literalSafe(TypeParameter c, TypeDeclarer t) {
        ...
        if (t instanceof GenericTypeDeclarer gtd) {
            ...
            var ov = c.view();
            var uv = gtd.param().view();
            if (ov.none() || uv.none()) return false;
            return uv.get().fields().keys().stream()
                    .allMatch(k -> ov.get().fields().exists(k));
        }
        ...
        for (var f : fields) {
            if (ov.has() && ov.get().fields().exists(f.name())) continue;
    ...
    private GenericTypeDeclarer analyse(GenericTypeDeclarer td) {
        if (td.kind().has()) {
            // *T / *?T / *#T: 只有值类型约束可引用
            var ov = td.type().param().view();
            if (ov.none()) {
                return semantic("only value-type support refer: %s", td.pos());
            }
            var view = ov.get();
            if (view.isRefer() == null || view.isRefer()
                    || view.domain() == null
                    || view.domain() == TypeDomain.FUNC
                    || view.domain() == TypeDomain.ATTRIBUTE
                    || view.domain() == TypeDomain.INTERFACE) {
                return semantic("only value-type support refer: %s", td.pos());
            }
        } else if (!td.required()) {
            // ?T: 仅 func 约束
            var ov = td.type().param().view();
            if (ov.none() || ov.get().domain() != TypeDomain.FUNC) {
    ...
    private void analyse(ParameterSet ps, boolean addVar) {
        for (var p : ps.params()) {
            if (p instanceof VariadicParameter vp) {
                // Variadic parameter: create a placeholder variable for the name.
                // The actual type checking happens at the call site.
                if (addVar) {
                    var v = new Variable(p.pos(), Modifier.empty(),
                            Declare.CONST, vp.name(),
                            Lazy.of(Primitive.BYTE.declarer()), Lazy.nil());
                    context.putVar(v);
                }
                continue;
            }
    ...
    private TypeValid assignable(
        ...
        if (ld instanceof ClassDefinition lc) {
            ...
            if (rd instanceof ClassDefinition rc) {
                var ok = ObjectTool.checkInherited(lt.derivedType(),
                                rt.derivedType())
    ...
    private boolean literalInitable(GenericTypeDeclarer gtd) {
        var ov = gtd.param().view();
        if (ov.none()) return false;
        var view = ov.get();
        if (!view.fields().isEmpty()) return true;
        if (view.hasRequiredInit()) return false;
        var d = view.domain();
        return d == TypeDomain.CLASS || d == TypeDomain.STRUCT;
    }

    private IdentifierMap<? extends Field>
    findFields(TypeDeclarer td, Entity e) {
        if (td instanceof VoidTypeDeclarer) return unreachable();

        if (td instanceof GenericTypeDeclarer gtd) {
            var ov = gtd.param().view();
            if (ov.has()) return ov.get().fields();
        }
        ...
        return semantic("type '%s' can't define fields: %s",
                td, e.pos());
    }
    ...
    private Groups.G2<Operand, TypeDeclarer> optimize(FieldOperand op) {
        ...
        Optional<? extends Field> of = findFields(sg.b(), op).tryGet(name);
    ...
    private Groups.G2<Expression, TypeDeclarer> optimizeField(
            PrimaryExpression s, Aggregatable<? extends Field> d,
            Identifier name, GenericMap gm, boolean expectCallable) {
        var o = d.field(name);
        if (o.none()) return semantic("'%s' not defined field '%s': %s",
                d, name, s.pos());

        var f = checkExport(o.get(), name);
        var t = analyse(gm.mapIf(f.type()));
    ...
    private Groups.G3<Expression, TypeDeclarer, Boolean> optimizeMethod(
            PrimaryExpression s, Abstractable<? extends Method> d,
            Identifier name, GenericMap gm, TypeArguments tas) {
        var o = d.method(name);
        if (o.none()) {
        ...
        if (m.generic().isEmpty() && !tas.isEmpty()) {
            return semantic("'%s' is not generic method: %s", m, name.pos());
        }
        checkExport(m, name);
        checkEscaped(st, m, name);
        checkUnmodifiable(s, m, name);
        checkEnterAsync(st, m, s);
        gm = checkConstraint(GenericMap.make(name, false, gm,
                m.generic(), tas));
    ...
    optimizeMember(PrimaryExpression s, ClassDefinition cd,
        ...
        if (om.has()) {
            ...
            checkUnmodifiable(s, m, name);
            checkEnterAsync(st, m, s);
            var gm = checkConstraint(GenericMap.make(name, false, st.gm(),
                    m.generic(), generic));
            var prot = gm.instantiate(m.prototype());
            var t = new AnonFuncTypeDeclarer(s.pos(), true, prot);
    ...
    private Groups.G2<Expression, TypeDeclarer> optimize(MemberOfExpression e) {
        ...
        if (sg.b() instanceof ArrayTypeDeclarer atd) {
            invalid(e.generic());
            return optimizeArray(s, atd, e.member(), e.expectCallable());
        }

        if (e.expectCallable()) {
            if (sg.b() instanceof DerivedTypeDeclarer dtd) {
                if (dtd.def() instanceof Abstractable<?> ad) {
                    var g = optimizeMethod(s, ad, e.member(), dtd.gm(), e.generic());
                    if (g.c()) return g.reduce();
                }
            } else if (sg.b() instanceof GenericTypeDeclarer gtd) {
                var ov = gtd.param().view();
                if (ov.has()) {
                    var g = optimizeMethod(s, ov.get(), e.member(),
                            GenericMap.EMPTY, e.generic());
                    if (g.c()) return g.reduce();
                }
            }
        }
        ...
        if (sg.b() instanceof EnumTypeDeclarer etd) {
            return optimizeField((PrimaryExpression) sg.a(), etd.def(), e.member(),
                    GenericMap.EMPTY, false);
        }

        if (sg.b() instanceof DerivedTypeDeclarer dtd) {
            if (dtd.def() instanceof Aggregatable<?> ad)
                return optimizeField(s, ad, e.member(), dtd.gm(),
                        e.expectCallable());
        } else if (sg.b() instanceof GenericTypeDeclarer gtd) {
            var ov = gtd.param().view();
            if (ov.has()) {
                return optimizeField(s, ov.get(), e.member(),
                        GenericMap.EMPTY, e.expectCallable());
            }
        }
    ...
    private Void inferNewType(NewExpression e, DerivedType dt) {
        ...
        var o = ObjectTool.checkInherited(etd.derivedType(), dt);
    ...
    analyseNewType(NewExpression e, GenericType gt) {
        ...
        if (view.domain() == TypeDomain.FUNC ||
                view.domain() == TypeDomain.ATTRIBUTE ||
                view.domain() == TypeDomain.INTERFACE) {
            return semantic("%s type not support new instance: %s",
                    view.domain(), gt.pos());
        }
        var ref = Optional.of(new Refer(
                e.pos(), STRONG, true, false));
        ...
        if (e.arg().none()) {
            if (!Boolean.TRUE.equals(view.newable())) {
    ...
    findCallable(SymbolExpression re) {
        ...
        if (s.module().none() && enterClass != null) {
            ...
            if (om.has()) {
                ...
                gm = checkConstraint(GenericMap.make(re, false, gm,
    ...
    private Groups.G2<Expression, TypeDeclarer> optimize(ObjectExpression oe) {
        ...
        if (t instanceof DerivedTypeDeclarer dtd &&
                dtd.def().domain() == TypeDomain.UNION &&
                oe.entries().size() > 1) {
    ...
    expand(CallExpression ce) {
        ...
        var va = context.variadic();
        if (va.none() && ce.variadic()) {
            return semantic("variadic arguments not defined: %s", ce.pos());
        }
        var args = Stream.concat(ce.arguments().stream(), va.stream()
                        .flatMap(a -> a.arguments().stream()))
                .toList();
        // First analyze all arguments
        var ags = args.stream().map(this::optimize).toList();
        ...
        var fg = ags.get(1);
        var sl = resolveFormatString(fg);
        if (sl == null) {
            return semantic("requires format string: %s", fg.a().pos());
        }
        ...
        if (ags.size() < fmt.argNum()) {
            ...
            return semantic("redundant arguments: %s",
                    ags.get(fmt.argNum()).a().pos());
        }
    ...
    /**
     * Extracts a compile-time string literal from an expression for format expansion.
     * <p>
     * When a format string is passed through a constant variable reference
    ...
    private StringLiteral resolveFormatString(
        ...
        if (fg.a() instanceof LiteralExpression le &&
    ...
    /**
     * Inline a variadic function body into a BlockExpression.
     * <p>
     * All caller arguments are converted to variable declarations at the start,
     * and parameter references in the inner call are replaced accordingly.
     * The resulting statements are analyzed in a new scope before returning.
     */
    private BlockExpression expandInlined(
            FunctionDefinition fd,
            CallExpression ce,
            Position pos) {
        var ps = fd.prototype().parameterSet();
        var params = ps.params();
        var args = ce.arguments();
        // Validate caller argument count
        if (args.size() < params.size())
            return semantic("missing arguments: %s", pos);

        // Optimize caller arguments to get their types
        var callerAgs = args.stream().map(this::optimize).toList();

        // Create variables for ALL caller arguments
        var dclVars = new ArrayList<Variable>();
        for (int i = 0; i < params.size(); i++) {
            var fp = ps.fixed(i);
            var fv = fp.var();
            if (fv.none()) continue;
            var v = fv.get().mirror();
            var optArg = callerAgs.get(i).a();
            v.value().set(optArg);
            dclVars.add(v);
        }
        // Store variadic args for body calls to expand
        if (ps.variadic()) {
            var varArgs = new ArrayList<Expression>(
                    callerAgs.size() - params.size());
            for (int i = params.size(); i < callerAgs.size(); i++) {
                varArgs.add(callerAgs.get(i).a());
            }
