...
这个表达式是返回数组的切片，但返回的是[数组](#变长数组)[虚引用](#虚引用类型)类型，所以要求被切片的数组一定能被虚引用。

表达式语法：数组 `[` 起始位置 `:` 结束位置 `]` 。
切片将包含起始位置到结束位置的前一个的元素（不包含结束位置），就是说其长度为：结束位置 - 起始位置 。
显然起始位置是\[0, 长度)，而结束位置是\[0, 长度\] 。
...
func test() {
    ...
    var b = new([8]int);
    const y [&]int = b[2:4];
    const z = b[2:4];           // 可以省略类型声明
    const w [&#]int = b[2:4];   // 或者转为不可修改类型
}
```
...
```feng
func exec(@Sync a *int, b *int) {   // 注意`exec`并不是并发边界
   startThread(a);                  // ✔：显然参数a已标注同步，可以穿过
   // startThread(b);               // ✖：参数b未标注同步，不可以穿过
}
```
...
func test() {
   ...
   @Sync var i = new(int);
   exec(i);                // ✔：变量i和参数a都是已同步的，可以传值

   var j = new(int);
   // exec(j);             // ✖：而变量j未同步，不可以传值
}
```
