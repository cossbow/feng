...
1. 变量是引用类型的。
2. 能映射成具体类型来使用。
3. 运行时检查边界，任何越界都抛出[异常](#异常)。
4. 能进行只读控制。

### mem引用类型

mem类型的变量是[引用](#引用类型变量)，支持强引用和虚引用。

#### mem强引用

由于[强引用类型](#强引用类型)，其引用的mem的实例需要用`new`创建。
...
[虚引用](#虚引用类型)在引用值类型实例的时候，限制只能引用支持[映射](#mem的映射)的类型。
例如：
...
空值就是`nil`，表示引用类型的变量或字段初始值，即不指向任何实例。
