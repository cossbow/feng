...
[引用](#引用)类型的变量和实例是分离的，这种实例则都是通过`new`创建的。
...
它要么不指向任何实例（其值等于`nil`），要么指向一个兼容类`Device`的实例，兼容性在[继承与多态](#继承与多态)中描述。
...
与[虚引用](#虚引用类型)的最基本区别是：
被强引用变量引用的实例是在使用中的，不能被内存管理器回收；当一个实例没有被强引用变量引用时就应该被回收。
