...
数组长度是指能容纳的元素总数，在声明时指定或者不指定，会声明两种不同的变量。
...
但结构类型的字段类型不能为引用，因此不指定长度的数组有另外的语义——弹性数组。
弹性数组的长度是可以计算出来的：当memory类型映射到结构体时，超出去的部分除以数组元素大小就是长度。
...
func alloc() {
    var r ram = new(ram, 32);
    var m ram`Message` = r;
    m.list[0] = 1; // list字段长度为：(32-8)/8
}
```
...
```feng
func main() {
   // 创建一个256字节大小的memory实例
   var buf ram = new([256]uint8);
}
```
