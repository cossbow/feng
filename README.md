...
考虑如果要改进C++语言，或者是改进版的C++，应该是优先考虑内存安全问题。引入了面向对象之后，对象之间互相引用会形成一个
复杂的图结构，对何时释放对象是个复杂的问题。同时对象的字段可能是指针，就需要考虑类型安全。

主要的特性如下：

1. 内存安全机制：
    1. 自动内存管理：指针只能通过`new`获取，并且释放（可能是延迟回收）时必须与指针清零同时存在。这不仅接近内存泄露问题，
       还有效解决Use-After-Free问题。
    2. 指针安全使用：指针不能通过运算生成，不能将任意整数转换成指针值，这也包括对指针进行自增自减。
    3. 强制类型检查：在遇到类型转换时必须检查是否允许转换，即使编译时无法分析时，也要在运行时做检查。
    4. 取地址操作：这就不支持了。不过在后面有虚引用来代替取地址操作。
2. 面向对象：继承与多态，单继承；接口抽象。
3. 内存结构：struct和union。
...
5. 值类型变量，可以帮助优化内存分配的性能：
    ...
    2. 在变量或字段所在的地方分配内存。
    3. 值类型的赋值是复制整个类型的数据，因为两个值类型的变量或字段实际上是两个内存区域。
...
包名称与目录名称一致，无声明，且目录名称的组成与变量命名要求一致，要是目录名不合规就不管吧。
...
1. 在包内部所有的内容都可见，跨包只能访问导出的符号。
2. 包名称与路径一一对应，在文件中不声明包名称。要求目录名称的规则和变量名称一样。
   例如在Linux下，包`com\jjj\base\util`对应的相对路径为`com/jjj/base/util`。
...
等级2的运算只有幂运算`^`，它的优先级高于其他二元运算，与等级1的一元运算的优先级为：
...
相当于C++的`this`指针，所以对值类型变量的实例，`this`如果传递出去会导致安全问题：
...
可以用于自动释放其他资源。比如C语言实现的缓冲区是需要手动分配和释放的，链接的自动关闭等等，这样就可以自动释放缓冲区了：
...
显然，结构类型字段和结构类型数组的元素、基本类型数组的元素都不会是其引用类型，自然不会形成循环引用了，所以完全可以采用引用计数来管理memory类型。
...
1. 虚引用变量只能用`const`声明。
   ```feng
   ```
2. 只能引用值类型变量（`var`类型）和引用类型常量（`const`）
   引用[值类型常量](#常量)，那常量的约束就无效了，可以通过引用去修改。引用语法没有能让实例只读的？
   引用类型变量变化后，虚引用会出现歧义：是继续引用原来的实例还是新的？
3. 显然虚引用不能传递引用给强引用。
...
```feng
func use(v &Vector) { // v是不可变的常量
   // TODO：……省略使用的代码
}
func test1(vec1 Vector, vec2 *Vector) { // 参数都是常量
   const v1 &Vector = vec;
   use(vec);
   // 或者:
   use(v1);
