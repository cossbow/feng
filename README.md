...
用于动态创建类、mem及数组的实例。参数是可选的。

1. 创建类实例时，如果没有带初始化表达式，就把其内存空间清零。
2. 创建变长数组实例时，类型参数中必须指定需要分配的数组长度。元素类型为类、引用、枚举时、接口、函数原型时自动清零。
3. 创建mem实例则不清零。
...
2. 内置字段`size`可以获取数组长度，类型为`int`。
3. 数组支持[索引运算](#索引运算符)。
4. 在索引元素时会进行边界检查，保证内存安全。
...
声明时如果指定了大小是定长数组，这个类型的变量是[值类型变量](#值类型变量)。
下面举例说明几个性质：

```feng
var a [4]int; // 未初始化时元素值是元素类型的默认值
var b [4]int = [0,1,2,3,4]; // 初始化的元素不能超过声明长度
// var c [8]int = b;    // 错误✖：长度不同不能赋值
var cs [4]Cat;     // 元素为自定义类Cat的数组
var crs [4]*Cat;   // 元素为引用的数组
var cas [4][8]Cat; // 元素为定长引用，也就是二维数组
var cas [4][]Cat;  // 元素为变长数组
```

未初始化时，元素均填充默认值零或`nil`：
```feng
var a [4]int;
var b [4]Cpu;
var c [16]*Bus;
...
注意：[结构类型](#结构类型)的字段类型不能为[引用](#引用)，必须指定长度。

## mem类型

mem是一段连续的内存空间，用于保存任意数据，比如从文件读取的数据等。

1. 只能通过`new`创建mem实例，且变量也是引用类型的。
2. mem只能复制数据，不能通过索引来读写，而只能映射成具体类型来使用。
3. 内置长度（字节数），并自动检查边界，越界则抛出[异常](#异常)。
4. 能进行只读控制。

### 创建mem实例

mem类型的变量是[引用](#引用)，而mem的实例也是用`new`创建的。
...
支持映射的类型：[结构类型](#结构类型)、[基本类型](#基本类型)和这两个类型的[数组](#数组)。
...
引用类型可以用`const`/`var`定义。`const`的引用是指变量本身不可变，而不是指被引用的实例（详细讨论在[常量](#常量)中）。
...
使用`new`创建的实例（包括类、mem及数组的实例），必须用对应类型的引用变量或字段引用，否则该实例是无用的，需立即清理或稍后清理。
...
虽然可以通过传入的[引用](#引用)来修改外部的实例，但实际大家更倾向于无副作用（不修改外部实例）。
...
[赋值](#赋值语句)只能修改变量本身的值，对引用类型变量，只能修改其引用的实例或者不引用任何实例（`nil`）。
引用类型的变量可以通过复制语句修改实例内容，也就是把右边的表达式值传递给实例：
...
类型安全需要在每种类型下分别讲述，比如类的实例有继承和抽象的约束、结构类型与mem及数组等。
...
3. 显然虚引用不能传递引用给强引用。
4. 虚引用只能用于[本地变量](#本地变量)，比如[返回值](#返回值表)和类[字段](#字段)的类型不能是虚引用。
...
字符串并不是基本类型，编译器对字符串字面量进行编码，创建[mem类型](#mem类型)的`rom`实例来保存。
