...
1. 内存安全机制的设计：
    1. 自动内存管理：指针必须指向一个对象或者为空，而对象在没有被指针时将被释放。主要解决内存泄露和Use-After-Free的问题。
    2. 指针安全使用：指针不能通过运算生成，不能将任意整数转换成指针值，这也包括对指针进行自增自减。
    3. 强制类型检查：在遇到类型转换时必须检查是否允许转换，即使编译时无法分析时，也要在运行时做检查。
    4. 虚引用：用于代替C语言取地址的操作，只能在实例声明周期内引用。
2. 面向对象的基本元素，包括继承多态和接口抽象。
3. 可以映射一段连续内存区的struct和union。
4. 有类似C++析构函数的实现，可用回收底层库分配的资源，或处理循环引用等。
5. 模块化代码管理，内部不进行隐藏和隔离，模块之间需要导出和导入。
...
8. 泛型：泛型参数可以加约束条件，类成员方法也可以带参数。
9. 异常机制：异常机制可以方便的处理设计之外的错误，避免使用goto处理。
